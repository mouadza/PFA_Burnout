...
class _FatigueCameraScreenState extends State<FatigueCameraScreen>
  ...
  Future<void> _captureAndSend() async {
    ...
    try {
      // ✅ stop live preview while capturing/sending
      if (!_previewPaused) {
      ...
      // ✅ Force proper image content-type
      ...
      final String riskTitle = data["risk_title"] ?? "Résultat";
      final String riskLabel = data["risk_label"] ?? "Moyen";
      final String message = data["message"] ?? "";
      final int score = (data["fatigue_score"] ?? 0) as int;

      final Map<String, dynamic> fatigueProfile =
      (data["fatigue_profile"] ?? {}) as Map<String, dynamic>;
      ...
      // ✅ when coming back: resume preview
      if (mounted) {
      ...
      // try to resume preview if error
      try {
        if (_controller != null && _controller!.value.isInitialized) {
          await _controller!.resumePreview();
          _previewPaused = false;
    ...
    return Scaffold(
      ...
      body: _loadingCamera
        ...
        child: Padding(
          ...
          : Column(
        children: [
          // ✅ Square camera preview (cadrage carré)
          Expanded(
            child: Center(
              child: AspectRatio(
